...
def train_model(X_train, X_test, y_reg_train, y_reg_test,
    ...
    with torch.no_grad():
        ...
        # Also get training predictions for threshold calibration
        reg_tr_pred, cls_tr_logits = eval_model(X_tr_t)
        cls_tr_probs = torch.sigmoid(cls_tr_logits).cpu().numpy()

    # ── Optimal threshold via Youden's J on TRAINING data ──
    # (finding threshold on test data would be data leakage)
    from sklearn.metrics import roc_curve as _roc_curve
    y_cls_train_hard = (y_cls_train > 0.5).astype(int)  # undo label smoothing
    fpr_tr, tpr_tr, thresholds_tr = _roc_curve(y_cls_train_hard, cls_tr_probs)
    j_scores = tpr_tr - fpr_tr
    optimal_idx = np.argmax(j_scores)
    optimal_threshold = thresholds_tr[optimal_idx]
    # Clamp to a reasonable range
    optimal_threshold = np.clip(optimal_threshold, 0.2, 0.8)

    cls_preds = (cls_probs > optimal_threshold).astype(int)
    print(f"\n    Optimal threshold (Youden's J on train): {optimal_threshold:.3f}")
