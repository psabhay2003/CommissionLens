...
class _ClsWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        # SHAP GradientExplainer expects 2D output (batch, num_outputs)
        # CommissionNet squeezes to 1D, so we unsqueeze it back
        if cls_logit.dim() == 1:
...
def run_shap_analysis(df=None):
    ...
    # GradientExplainer can return various shapes — normalise to 2D (samples, features)
    if isinstance(shap_values, list):
        shap_values = shap_values[0]
    if torch.is_tensor(shap_values):
        shap_values = shap_values.cpu().numpy()
    if shap_values.ndim == 3:
        shap_values = shap_values[:, :, 0]  # take first (only) output head
