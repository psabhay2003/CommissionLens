...
from config import (DATA_DIR, MODEL_DIR, REPORT_DIR,
                    SIP_MONTHLY_AMOUNT, TOP_K_FUNDS, TEST_QUARTERS)
...
def get_ensemble_proba(models, X):
    """Average prediction probabilities across all base classifiers."""
    probs = []
...
def run_sip_backtest(df=None):
    ...
    available = [c for c in feat_cols if c in test_data.columns]
    X = test_data[available].fillna(0)
    X_scaled = pd.DataFrame(scaler.transform(X), columns=available, index=X.index)

    test_data["model_prob"] = get_ensemble_proba(models, X_scaled.values)
    ...
    sns.boxplot(data=results_df, x="strategy", y="xirr", palette=palette, ax=axes[0])
